...
class RoutesTest(asynctest.TestCase):
    async def test_calls_index_bulk_passing_received_message_objects(self):
        ...
        with mock.patch.object(
            routes.indexer, "bulk", indexer_bulk_mock
            ...
            expected_logger_function_call = mock.call(
                conf.logger, 2, "bulk_index_time", mock.ANY
            )
            self.assertEqual(
                [expected_logger_function_call],
                logger_function_mock.await_args_list,
            )
    ...
    async def test_app_uses_right_configs(self):
        ...
        with mock.patch.dict(
            ...
            self.assertEqual(
                "myvhost",
                routes.app.routes_registry[routes.generic_app_log_indexer][
                    "options"
                ]["vhost"],
            )
            self.assertEqual(
