...
class LogIndexerTest(asynctest.TestCase):
    ...
    def test_test_generate_correct_index_name(self):
        ...
        """
        self.assertEqual(
            "asgard-app-logs-infra-asgard-logs-counts-2018-06-27-13",
        ...
        self.assertEqual(
            "asgard-app-logs-infra-asgard-logs-counts-2018-06-27-13",
            self.indexer._index_name(
                {"key": "errors.asgard.app.infra.asgard.logs.counts"}
            ),
        )
    ...
    async def test_extracts_appname(self):
        self.assertEqual(
            "/infra/apps/myapp",
            self.indexer._app_name_with_namespace(
                {"key": "asgard.app.infra.apps.myapp"}
            ),
        )
        self.assertEqual(
            "/infra/otherapp/app",
            self.indexer._app_name_with_namespace(
                {"key": "errors.asgard.app.infra.otherapp.app"}
            ),
        )
        self.assertEqual(
