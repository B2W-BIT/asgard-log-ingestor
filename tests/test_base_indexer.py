...
class BaseIndexerTest(asynctest.TestCase):
    def setUp(self):
        ...
        self.logger_mock = mock.CoroutineMock(
            info=mock.CoroutineMock(), error=mock.CoroutineMock()
        )
    ...
    async def test_call_extract_appname_when_needed(self):
        ...
        self.assertEqual(
            "/infra/asgard/collector", indexer._app_name_with_namespace({})
        )
    ...
    async def test_calls_index_bulk_passing_received_message_bodies(self):
        ...
        self.assertEqual(
            expected_bulk_data,
            list(self.elasticsearch_mock.bulk.await_args_list[0][0][0]),
        )
    ...
    async def test_generated_right_action_data_for_bulk_insert(self):
        ...
        with mock.patch.object(conf, "BULK_INSERT_TIMEOUT", 60):
            returned_by_elasticsearch = await indexer.bulk(
                [
                    CoroutineMock(
                        body={
                            "timestamp": 1_530_132_385,
                ...
                mock.call(
                    [
                        ...
                        },
                        {
                            "key": "errors.asgard.app.sieve.captura.kirby.powerup",
                            "some-value": 10,
                            "timestamp": "2018-12-17T15:01:27+00:00",
                            "appname": "myapp",
                            "asgard_index_delay": mock.ANY,
                        },
                    ],
    ...
    async def test_if_result_does_not_have_errors_dont_iterate_it(self):
        ...
        with mock.patch.object(
            self.indexer, "bulk_", indexer_bulk_mock, create=True
        ):
            messages = [
                CoroutineMock(
                    body={
    ...
    async def test_should_index_a_different_document_for_messages_with_index_errors(
        self
    ):
        """
        ...
        expected_second_bulk_call = mock.call(
            [
                {
                    "index": {
                        "_index": indexer._index_name(messages[0].body),
                        "_type": "logs",
                    }
                },
                {
                    "asgard": {
                        "original": {"msg": json.dumps(expected_original_msg)},
                        "error": {
                            "type": "illegal_argument_exception",
                            "reason": "Cant merge a non object mapping [marketplace_sellers] with an object mapping [marketplace_sellers]",
                        },
        ...
        self.assertEqual(
            expected_second_bulk_call,
            self.elasticsearch_mock.bulk.await_args_list[1],
        )

    def test_prepare_indexed_document_add_delay_key_log_error(self):
        with freeze_time("2018-06-27T14:09:56+00:00"):
            prepared_document = self.indexer.prepare_document(
                self.logmessage_parse_error
            )
    ...
    def test_prepare_indexed_document_do_not_override_special_fields(self):
        self.maxDiff = None
        self.logmessage_parse_ok["payload"][
            "timestamp"
        ] = 1_530_120_546  # ~qua jun 27 17:28:57 UTC 2018
    ...
    def test_prepare_indexed_document_add_delay_key_log_ok(self):
        ...
        with freeze_time("2018-06-21T19:26:06+00:00"):
            prepared_document = indexer.prepare_document(
                self.logmessage_parse_ok
            )
            self.assertEqual(0, prepared_document["asgard_index_delay"])
