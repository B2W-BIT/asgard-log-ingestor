...
class Indexer:
    ...
    async def bulk(self, documents: List[RabbitMQMessage]):
        ...
        for doc in documents:
            _bulk.append(
                {
                    "index": {
                        "_index": self._index_name(doc.body),
                        "_type": "logs",
                    }
                }
            )
            _bulk.append(self.prepare_document(doc.body))
        result = await self.elasticsearch.bulk(
            _bulk, request_timeout=conf.BULK_INSERT_TIMEOUT
        )
            ...
            for idx, item in enumerate(result["items"]):
                if item["index"].get("error"):
                    ...
                    new_document = {
                        ...
                        "appname": self._app_name_with_namespace(
                            original_document.body
                        ),
                        "asgard_index_delay": prepared_document[
                            "asgard_index_delay"
                        ],
                    }
                    _second_bulk.append(new_document)
            await self.elasticsearch.bulk(
                _second_bulk, request_timeout=conf.BULK_INSERT_TIMEOUT
            )
        await self.logger.info(
            {
                "messages-processed": total_messages,
                "accepted-messages": accepted - rejected,
                "rejected": rejected,
    ...
    def prepare_document(self, document):
        ...
        timestamp = datetime.utcfromtimestamp(document["timestamp"]).replace(
            tzinfo=timezone.utc
        )
...
class AppIndexer(Indexer):
    ...
    def _index_name(self, document):
        app_name_with_namespace = (
            self._app_name_with_namespace(document).strip("/").replace("/", "-")
        )
